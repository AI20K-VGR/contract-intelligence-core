...
Từ root: tạo `infra/compose/.env` theo file mẫu, đặt mật khẩu/token ngẫu nhiên và đường dẫn artifact trên máy được phép, rồi chạy:

```powershell
.\scripts\demo.ps1 -Profile core
```

Image cài Tesseract `vie+eng`. Không inference external. Web `http://localhost:8080`, Swagger `http://localhost:8000/docs`. Token không lưu localStorage. Account cùng workspace, mutation theo role operator/reviewer/admin; mentor-readonly chỉ đọc. Chưa có ACL từng dossier.
...
# Sửa CI_DATABASE_URL, CI_ARTIFACT_ROOT, CI_ACCOUNTS trong .env.
...
| `document_processing.py` | Native geometry, canonical PNG, scan/mixed Tesseract |
...
| `reporting.py`, `policy.py` | Batch accounting, roles/authentication |
...
Mutation cần `Authorization: Bearer <token>` và `Idempotency-Key: <unique-key>`. Cùng actor/key/body trả response cũ; khác operation/body trả 409. Job ID cũng là run ID; retry tạo run mới, giữ snapshot cũ.
...
5. Resolve `/citations/{id}/resolve`; lấy page URL với Bearer token. Bbox normalized trên PNG sau rotation/crop.
...
Test tạo synthetic PDF trong thư mục tạm. Suite kiểm native end-to-end, bbox xoay/crop, scan routing (mock), review/approval, idempotency, roles, fencing, retry, batch, migration SQLite. PostgreSQL multi-worker, scan OCR thật và Docker runtime cần môi trường Docker/Tesseract hoạt động.
