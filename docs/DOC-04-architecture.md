...
| Downstream | DOC-05 API (v0.3.0), DOC-04b Schema (v1.2.0), DOC-04c ERD (v1.2.0), DOC-06 evaluation |
...
|---|---|
| Client → API | Bearer JWT, `X-Tenant-Id` isolation, RBAC (`x-rbac`), MIME/magic-byte, AV, size/page limit, idempotency |
...
Bảng dưới tham chiếu skeleton trên branch `feature/backend-setup`.
...
|---|---|
| Isolation | tenant (`X-Tenant-Id`) + containment trước RBAC (`x-rbac`); storage/cache key theo tenant |
...
DOC-05 là authority cho đường dẫn, request/response DTO, RBAC và mã lỗi HTTP. DOC-04 chỉ định boundary và invariant; implementation không được tự mở endpoint OCR direct để “cho tiện demo”. Toàn bộ API áp dụng cơ chế xác thực **Bearer JWT**, định danh đa khách hàng **`X-Tenant-Id`** và phân quyền nghiêm ngặt theo **`x-rbac`**.

| Nhóm API | Năng lực & Endpoints chính | Invariant kiến trúc & Phân quyền x-rbac |
|---|---|---|
| **Intake (Dossiers)** | `POST /dossiers` (tạo dossier + multipart upload)<br>`GET /dossiers` (danh sách dossier phân trang)<br>`GET /dossiers/{id}` (chi tiết dossier)<br>`PATCH /dossiers/{id}` (cập nhật metadata)<br>`POST /dossiers/{id}/lock` (khóa thẩm tra)<br>`POST /dossiers/{id}/reprocess` (chạy lại) | Source chỉ tạo version mới; manifest pin document/source digest; `dossier_id` gắn chặt với `tenant_id`.<br>`x-rbac`: `OPERATOR`, `ADMINISTRATOR` (tạo, sửa, reprocess); `OPERATOR`, `REVIEWER`, `ADMINISTRATOR` (xem); `REVIEWER`, `ADMINISTRATOR` (lock). |
| **Documents** | `GET /dossiers/{id}/documents` (danh sách văn bản)<br>`GET /documents/{id}` (chi tiết văn bản)<br>`GET /documents/{id}/content` (tải/stream file PDF gốc) | File nhị phân stream qua Object Storage; URL scoped, short-lived và tenant-bound.<br>`x-rbac`: `OPERATOR`, `REVIEWER`, `ADMINISTRATOR`. |
| **Manifest Confirmation** | `GET /dossiers/{id}/manifest` (lấy manifest dự thảo)<br>`POST /dossiers/{id}/manifest/confirm` (xác nhận manifest) | Operator chính thức thẩm định và chốt cấu trúc hợp đồng chính/phụ lục, số hiệu, ngày ký trước khi so sánh.<br>`x-rbac`: `OPERATOR`, `ADMINISTRATOR` (xác nhận); `OPERATOR`, `REVIEWER`, `ADMINISTRATOR` (xem). |
| **Runs (Pipeline Runs)** | `POST /dossiers/{id}/runs` (kích hoạt run mới)<br>`GET /runs` (danh sách runs)<br>`GET /runs/{id}` (chi tiết run/stages/metrics)<br>`POST /runs/{id}/cancel` (huỷ run) | Rerun là whole-dossier tạo snapshot mới; task table điều phối qua dispatcher.<br>`x-rbac`: `OPERATOR`, `ADMINISTRATOR` (trigger, cancel); `OPERATOR`, `REVIEWER`, `ADMINISTRATOR` (xem). |
| **Batches** | `POST /batches` (tạo batch từ ZIP + manifest)<br>`GET /batches` (danh sách)<br>`GET /batches/{id}` (chi tiết batch và dossiers)<br>`GET /batches/{id}/summary` (tóm tắt kết quả/chi phí)<br>`POST /batches/{id}/cancel` (huỷ batch)<br>`POST /batches/{id}/resume` (tiếp tục batch) | Membership bất biến; action không ảnh hưởng item khác; ETag CAS.<br>`x-rbac`: `OPERATOR`, `ADMINISTRATOR` (tạo, cancel, resume); `OPERATOR`, `REVIEWER`, `ADMINISTRATOR` (xem). |
| **Re-OCR** | `POST /documents/{id}/re-ocr` (đặt hàng OCR lại)<br>`GET /documents/{id}/re-ocr-requests` (lịch sử) | Hạn ngạch hữu hạn; profile chuyên sâu (`high_res_binarize`, `table_optimized`); backend kiểm soát grant.<br>`x-rbac`: `OPERATOR`, `REVIEWER`, `ADMINISTRATOR`. |
| **Evidence & IDP** | `GET /documents/{id}/pages`<br>`GET /documents/{id}/clauses`<br>`GET /documents/{id}/tables`<br>`GET /dossiers/{id}/facts`<br>`GET /dossiers/{id}/findings`<br>`GET /dossiers/{id}/conflicts`<br>`GET /citations/{id}` | Pin `snapshot_id`/`run_id`; CPS coordinates `[x0,y0,x1,y1]`; quote Unicode exact; không serialize float.<br>`x-rbac`: `OPERATOR`, `REVIEWER`, `ADMINISTRATOR`. |
| **Review & Revisions** | `GET /dossiers/{id}/review-items`<br>`GET /review-items/{id}`<br>`POST /review-items/{id}/actions`<br>`GET /review-items/{id}/revisions` | Append-only review revisions; optimistic concurrency với `base_version` (trả `409` khi conflict).<br>`x-rbac`: `REVIEWER`, `ADMINISTRATOR`. |
| **Approval** | `POST /dossiers/{id}/approve` | Sign-off nội bộ của Reviewer/Admin khi mọi review item đã giải quyết.<br>`x-rbac`: `ADMINISTRATOR`. |
| **External Approvals** | `POST /dossiers/{id}/external-approvals`<br>`GET /dossiers/{id}/external-approvals`<br>`POST /external-approvals/callback` | Tích hợp cổng ký ngoài (DocuSign, SAP, ERP Portal); verify chữ ký số và token callback.<br>`x-rbac`: `ADMINISTRATOR` (dispatch grant & webhook callback); `OPERATOR`, `REVIEWER`, `ADMINISTRATOR` (xem). |
| **Optimization Loop** | `GET/POST /optimization/campaigns`<br>`GET/POST /optimization/candidates`<br>`POST /optimization/candidates/{id}/promote`<br>`GET/POST /optimization/experiments`<br>`POST /optimization/experiments/{id}/run`<br>`GET /optimization/experiments/{id}/results` | Tự động hóa benchmark đánh giá prompt/heuristic trên Golden Dataset; chỉ promote candidate vượt trội.<br>`x-rbac`: `ADMINISTRATOR`. |
| **Audit & Ops** | `GET /dossiers/{id}/audit`<br>`GET /dossiers/{id}/export`<br>`GET /ops/summary` | Audit trail đầy đủ dòng thời gian; KPI vận hành (throughput, cost, failure); không rò rỉ PII/source trong telemetry.<br>`x-rbac`: `ADMINISTRATOR` (ops); `OPERATOR`, `REVIEWER`, `ADMINISTRATOR` (audit, export). |
...
| **Probes** | `GET /healthz`, `GET /readyz` | Liveness & Readiness probe (DB, MinIO, disk). Không yêu cầu auth. |
...
sequenceDiagram
  ...
  U->>G: Request + Bearer JWT + X-Tenant-Id + Idempotency-Key
  G->>R: Resolve resource và parent path
  R-->>G: tenant + containment result
  G->>P: Check tenant match, x-rbac role, data classification
  P-->>G: allow/deny
  alt allow
    ...
    G-->>U: 401/403/404/409/422 Problem Details
  end
```
...
| DOC-05 public contract | containment/RBAC/CAS/internal boundary, 9 nhóm enterprise modules |
...
| 2026-09-17 | v0.4.0 | ADR-04: object storage development từ filesystem local sang MinIO S3-compatible. | Khớp skeleton backend và profile compose `local-baseline`. | NFR-01 local demo. | DOC-04 §18.2, README. | Backend |
| 2026-09-17 | v0.3.0 | Re-OCR: tách rõ `state` và `resolved_route`; thêm `QUARANTINED`; route `DENIED` ⇒ state `FAILED` + `outcome_code`. | DOC-04 state diagram dùng route như state, thiếu `QUARANTINED` so với schema/DOC-05. | BR-03, BR-06. | DOC-04 §9, `reocr-request.v3.schema.json`, `contracts/README.md`, fixtures, DOC-05. | Backend + AI2 |
| 2026-09-17 | v0.2.0 | Page ledger: thêm `quality.coverage_status = BLANK_VERIFIED` vào `ai1.snapshot.v3` và bảng ánh xạ snapshot → ledger. | ADR-11/DOC-06 yêu cầu `BLANK_VERIFIED` nhưng contract AI1 không có cách khai báo. | BR-02, BR-03, DOC-06 §6. | DOC-04 §6, snapshot v3 schema, fixtures, `contracts/README.md`. | AI1 + Backend |
| 2026-09-17 | v0.1.0 | Khởi tạo tài liệu kiến trúc sơ bộ. | Sprint 1 initial baseline. | Toàn bộ tài liệu. | DOC-01..03. | Team |
