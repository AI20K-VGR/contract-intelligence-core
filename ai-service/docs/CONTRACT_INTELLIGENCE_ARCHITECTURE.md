...
| **Quyết định đã chốt** | Engine OCR chính = **OpenAI GPT-5.6 "Terra"** — vision API, bản giữa dòng GPT-5.6 |
...
- **Native-first, OCR-second.** PDF có text-layer dùng thẳng (PyMuPDF), không gọi OCR/LLM — tái dùng nguyên bộ phân loại trang (`classify_pdf.py`) đã build và đo ở Sprint 1. Đây là cơ chế kiểm soát chi phí quan trọng nhất vì GPT-5.6 Terra tính phí theo ảnh/token.
...
| GPT-5.6 Luna (Lunar Light) | API ngoài | ~$0.20/1M input – $1.20/1M output (~10x rẻ hơn Terra) | Tương đương hoặc nhanh hơn Terra (model nhẹ hơn) | Lựa chọn dự phòng để giảm chi phí nếu ngân sách API bị siết ở Sprint 3; đổi qua tham số `model`, không cần code mới |

### 4.2 Lý do chọn Terra

- **Độ chính xác ưu tiên hơn chi phí cho tài liệu pháp lý.** Hợp đồng thương mại chứa số tiền, ngày tháng, mã số thuế, số điều khoản — sai một ký tự có thể đổi nghĩa pháp lý. Terra là model giữa dòng (không phải bản rẻ nhất), giảm rủi ro đọc sai trên chữ nhỏ/mờ so với Luna, vốn là bản nhẹ được tối ưu cho chi phí hơn độ chính xác.
- **Đã là mặc định của adapter.** `DEFAULT_MODEL = "gpt-5.6-terra"` trong `openai_vision_ocr.py` — không cần sửa code, không phát sinh rủi ro kỹ thuật khi triển khai.
- **Đã có số liệu đo thật từ Sprint 1** (tốc độ tuần tự/song song, hành vi tuân thủ prompt chống bịa trên ảnh scan mờ) — Luna chưa có số liệu tương đương, nên chọn Terra giảm rủi ro "chưa kiểm chứng" khi lên production.
- **Khối lượng hợp đồng ở quy mô OJT chưa đủ lớn để chi phí là yếu tố quyết định** — chênh lệch ~10x so với Luna vẫn nằm trong ngân sách API dự kiến cho Sprint 2–3 (cần Mentor xác nhận con số cụ thể, xem §16).
- Cùng chịu ràng buộc API `gpt-5.6+` (dùng `max_completion_tokens`, không chỉnh `temperature`) — adapter đã xử lý sẵn, không đổi khi dùng Terra.
- Luna vẫn được giữ làm phương án dự phòng: nếu chi phí Terra vượt ngân sách thực tế ở Sprint 3, có thể đổi sang Luna qua config mà không đổi orchestration — nhưng phải benchmark CER/WER trên bộ 30 mẫu gán nhãn trước khi đổi, đúng tinh thần Sprint 1: "quyết định engine bằng dữ liệu đo thực".
...
- Input: text/markdown thô theo trang (kể cả `raw_markdown` từ Terra) + geometry có sẵn.
- Dùng LLM (cùng model Terra, hoặc model text-only rẻ hơn nếu tách được OCR khỏi structuring) để bóc tách cây: `Chương → Điều → Khoản → Điểm`, và bảng trong phụ lục thành dạng có cấu trúc (rows/columns).
...
- Đây là nơi thu thập dữ liệu để sau này đánh giá lại độ chính xác của Terra so với Luna (phương án rẻ hơn) bằng dữ liệu thật (không phải dataset tổng hợp).
...
- Theo dõi chi phí API theo document (số trang gọi Terra × đơn giá) để có số liệu thật báo cáo Sprint Review, tránh lặp lại tình trạng "chi phí GPU/CPU chưa ước tính" đã ghi nhận ở Sprint 1.
...
- **Sprint 2:** dựng khung service (API + DB + queue), chuyển adapter GPT-5.6 Terra vào pipeline thật, build Structuring + Citation worker, HITL UI bản tối giản (approve/reject).
- **Sprint 3:** Conflict Detection worker, HITL UI đầy đủ (edit, audit trail), benchmark Terra vs Luna trên dữ liệu thật để xác nhận có cần đổi engine giảm chi phí không, hoàn thiện bảo mật (cờ sensitivity, secret management), chuẩn bị production-ready.
...
- `src/contract_ocr/infrastructure/ocr/openai_vision_ocr.py` — adapter OpenAI; `DEFAULT_MODEL = "gpt-5.6-terra"` đã đúng với quyết định này, không cần sửa code. Muốn thử Luna sau này chỉ cần truyền `model="gpt-5.6-luna"` qua config.
