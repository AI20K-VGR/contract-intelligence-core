# ai-service — contract-ocr-lab

> Phần Python OCR / IDP của monorepo `contract-intelligence-core` (xem [README gốc](../README.md)). Nhận request trích xuất từ `backend/` trong kiến trúc dự kiến; hiện tại đây là một service/pipeline OCR chạy độc lập, đã có CLI benchmark và một test UI thủ công (backend FastAPI + frontend tĩnh) để tự kiểm thử — **chưa nối REST vào `backend/`**, đó là việc tiếp theo.
