...
def _verify_internal_service_key(value: str | None) -> None:
    """Require the shared key only when deployment config supplies one."""
    configured = os.environ.get("AI_SERVICE_API_KEY")
    if configured and (value is None or not hmac.compare_digest(value, configured)):
...
@app.get("/healthz", tags=["Backend contract"])
def healthz() -> dict[str, str]:
    """Liveness endpoint consumed by backend's readiness check."""
    return {"status": "ok", "service": "contract-ocr-ai1"}
...
@app.post(
    ...
    summary="Submit an AI1 OCR job from the Contract Intelligence backend",
)
def submit_backend_ocr_job(
    request: BackendOcrJobRequest,
    background_tasks: BackgroundTasks,
    x_internal_service_key: str | None = Header(default=None),
) -> dict[str, Any]:
    """Accept a short-lived source URL and return an ID that backend can poll."""
    _verify_internal_service_key(x_internal_service_key)
    if not request.pages_to_process:
        raise HTTPException(422, "pages_to_process must not be empty")
    job_id, job = _new_backend_job("ocr")
    background_tasks.add_task(_run_backend_ocr, job_id, request)
    return {
        "job_id": job_id,
        "kind": "ocr",
        "status": job["status"],
...
@app.delete(
    "/api/v1/jobs/{job_id}",
    tags=["Backend contract"],
    summary="Cancel an AI1 OCR job that has not completed",
)
